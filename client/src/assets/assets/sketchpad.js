class Sketchpad {
  constructor(width, height, canvasID, rainbowMode) {
    ...
    this.imageData = { span: this.width, data: new Array(this.width * this.height).fill(0) };
    ...
    this.typeLogHead = 0
    this.typeLog[this.typeLogHead] = {"data":new Array(this.width*this.height).fill(0),
                                      "span":this.width,
  ...
  drawPix(event) {
    ...
    var [x, y] = [
      Math.round((event.clientX / this.canvas.clientWidth) * this.canvas.width),
      Math.round((event.clientY / this.canvas.clientHeight) * this.canvas.height)
    ];
  ...
  drawTo(event) {
    ...
    if (this.mousedown && !(this.lastMousePos[0] == -1 && this.lastMousePos[1] == -1)) {
  ...
  drawChar(char) {
    ...
    var newText = {"data":new Array(this.width*this.height).fill(0),
    ...
    if (this.cursorPos[0] + charWidth + this.textMargin > this.width) {
      ...
      if (this.cursorPos[1] + 2*this.lineSpacing + this.textMargin <=  this.height) {
    ...
    for (var i = 0; i < newData["data"].length; i+=4) {
      ...
      if (oldData["data"][i+3] != newData["data"][i+3] && newData["data"][i+3]>128) {
  ...
  backspace() {
    var nextTypeLogHead = (this.typeLogHead+this.charsLogged-1)%this.charsLogged
  ...
  getColour(i) {
    switch (i) {
      ...
      default:
         ...
         *         ^up        ^flat      ^down
         */
        var toCode = (r, g, b) => {return "rgba("+r+","+g+","+b+",1)"};

        var up = i => Math.round(255 - this.saturation + this.saturation * (i % 1));
        var flat = () => 255;
        var down = i => Math.round(255 - this.saturation + this.saturation * (1 - (i % 1)));
